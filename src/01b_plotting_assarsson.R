...
# Load processed data (output from 01a_load_assarsson_data.R)
df_merged <- read_csv("data/processed/merged_assarsson_data.csv") |>
...
# --- Classification Distribution: Full vs Mapped vs Leftover ---
...
# --- Epitope Selection Funnel (Assarsson et al. 2008) ---
df_funnel <- tibble(
  step = factor(c(
    "1. All 9/10-mers\n(~70,000)",
    "2. High-affinity\nbinders (2.5%)",
...
df_merged_assarsson72 |>
  ...
  scale_y_log10() +
  labs(
    title    = "Binding Affinity by Classification (excluding recoded <1 values)",
    subtitle = "Strong binding alone does not predict immunodominance",
    x        = "Classification",
    y        = "Affinity (nM, log scale)"
  ) +
...
# --- Processing vs Immunogenicity ---
df_merged_assarsson72 |>
  filter(!is.na(processed)) |>
  ggplot(aes(x = processed, y = immunogenic, color = classification)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_smooth(
    method      = "lm",
    se          = TRUE,
...
# --- Affinity: Processed vs Not Processed ---
p_dc_aff <- ggplot(
  df_merged_assarsson72,
...
# --- Processing Score: Processed vs Not ---
...
# --- Sequence Logos by Classification (9-mers) ---
...
# --- Physicochemical Feature Calculation ---
...
fviz_cluster(
  km_res,
  data    = scale(feature_matrix),
  palette = "jco",
  ggtheme = theme_minimal()
) +
  labs(title = "K-Means Clustering (Physicochemical Features)")

# --- Cluster vs Classification Heatmap ---
