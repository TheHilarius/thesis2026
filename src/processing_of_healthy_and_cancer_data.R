...
df_clean <- df_raw |>
  ...
  # remove epitope_ prefix from column names
  rename_with(~ str_remove(.x, "^epitope_"), starts_with("epitope_")) |>
  rename(
    start = starting_position, end = ending_position,
    peptide = name
    ) |>
  mutate(
    uniprot_id = str_extract(molecule_parent_iri, "[^/]+$")) |>
...
write_csv(df_clean, "data/pos_EL_all_epitopes_hla0201.csv")
